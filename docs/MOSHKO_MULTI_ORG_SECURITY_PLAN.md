...
| # | פער | חומרה |
|---|---|---|
| 1 | מיגרציה הנפיקה `CREATE TYPE userrole` → `DuplicateObject` בפרוד | **P0** — בלתי-נראה ב-SQLite |
...
### שלוש תיקוני-דוח (אומתו 11/08 ערב)

1. **סדר ה-DDL.** דיווחתי שהכשל מתרחש "אחרי ש-CREATE TABLE בוצע,
   כלומר במצב חצי-מיגרציה". **שגוי.** ה-SQL בפועל (worktree על `3cb12a4`):
   `CREATE TYPE` בשורה 5, `CREATE TABLE` בשורה 7. הכשל מתרחש
   **בהצהרה הראשונה**, לפני שנוצר דבר. עדיין P0 — המיגרציה חסומה
   לחלוטין — אבל **אין מצב חצי-מיגרציה** ואין צורך בהתערבות ידנית
   לניקוי. הערכת החומרה שלי הייתה מוגזמת.

2. **`policy_engine` אינו קיים בענף האבטחה.** דיווחתי שהוא "נשאר בלי
   persistence ובלי קוראים", כאילו הוא כאן. הוא נמצא רק על
   `chore/repo-order-and-control-plane` ולא הועבר לאף אחד משלושת
   הענפים החדשים. אין בענף הזה קובץ `policy_engine.py` ואין טסטים לו.

3. **מחזור החיים אינו מחובר ל-routes.** `grant_checked`, `revoke_checked`,
   `suspend_checked`, `invite` ו-`accept` נבדקים ברמת השירות בלבד —
   `grep` על `src/cfo/api/` מחזיר אפס. אין endpoint שמפעיל אותם, ולכן
   בקרת הסמכות והרישום ב-AuditLog אינם נאכפים על אף קריאת HTTP.
