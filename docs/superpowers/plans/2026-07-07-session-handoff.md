...
**עדכון סופי (2026-07-07, מ-SUMIT support):** נסגר — הנתיב API מת.
- **אין "מפתח משרד" כולל** ב-SUMIT. מפתחות הם פר-עסק (כל לקוח נפרד).
  לכן אין טעם לחפש מפתח משרד — הרעיון לא קיים ב-SUMIT.
- **"תיוק קבצים" לא זמין דרך API בכלל** — זו פעולת מסך בלבד, בתוך תיק
  הלקוח: "תיק הלקוח ← קבצים ← תיוק קבצים". OCR ממלא, הרו"ח משלים+שומר
  למנה. (add_expense שניסיתי היה גם הנתיב הלא-נכון: addexpense≠תיוק.)

**החלטת המשתמש (2026-07-07): "נמשיך עם זה בהמשך, נגדיר בינתיים, רק תתעד."**
צד SUMIT (תיוק 41 ההוצאות בספר הלקוח) **נדחה למועד מאוחר יותר** — לא הוגדר
עדיין איך (צוות המשרד ידני ב-UI, או browser-automation של מסך "תיוק קבצים").
**מצב מוקפא:** 41 ההוצאות קלוטות ורק ברצף (org5, מע"מ ₪32,171). 50 קבצי
המקור נשארים "ממתין לקליטה" ב-SUMIT ללא שינוי. אין כפילות, אין נזק.

**כשחוזרים לזה — 2 דרכים (שתיהן פתוחות, המשתמש יבחר):**
1. צוות המשרד מתייק ב-SUMIT UI (OCR-assisted, מומלץ) — רצף כבר מחזיקה את
   כל 41 הסכומים להצלבה מהירה (טבלת אימות בסשן / query org5 expenses).
2. Browser-automation: להיכנס ל"תיק הלקוח ← קבצים ← תיוק קבצים" בתיק עומר
   (1999386278) ולמלא כל אחד מ-41 (הנתונים מוכנים ב-DB + scratchpad/receipts).
   פיילוט-אחד-קודם. תהליך ארוך על ספר מס — לאמת לפני batch.
נתוני 41 ההוצאות: query `Expense WHERE organization_id=5` (external_id=
sumit_file_{uuid}), + קבצי מקור scratchpad/receipts/*.pdf + uuids.txt.
