...
- **הפרדה רב־ארגונית נסגרה אופליין 13/08:** כל בקשה ארגונית מוכרעת מחדש
  ...
  ירוקים. **לא פרוס**; שרשרת המיגרציות עד `05c6d7e8f9a0` ו־smoke בפרוד עדיין
...
- **מועמד הסגירה של מושקו והרשאות הושלם מקומית 13/08:** `policy_grants`
  ...
  שרשרת המיגרציות בעלת head יחיד `05c6d7e8f9a0`, ועברה upgrade, downgrade,
...
|---|---|---|---|
| 0.1 | **deploy לפרוד** של ה־PR המאושר → זרימת הסכימה המאומתת ב־`GATE0_DEPLOYMENT_RUNBOOK.md` → smoke | בעלים + מפעיל מורשה בלבד | 🟡 **חלקי.** קו הפרוד מ־09/08 עומד על **`a9b0c1d2e3f4`** עם drift ריק וספירות שמורות; מחזור cron של 10/08 הסתיים ב־05:01, 4 ריצות `COMPLETED`, אפס כפל sync. למועמד 13/08 יש head חדש **`05c6d7e8f9a0`** והוא עדיין לא מוזג, לא נפרס ולא הורץ בפרוד. לאחר פריסתו נדרש נוהל המיגרציה המעודכן, smoke ללא ספק, ורק באישור נפרד מחזור ידני חי שצורך מכסות SUMIT/OF. גיבוי הפריסה הקודמת: `neondb-pre-migrate-20260809-1539.sql`; לחלון החדש נדרש גיבוי טרי. |
...
- ממתין: מיגרציית פרוד (alembic upgrade head) — פעולת בעלים; ואימות בריאות אחרי deploy.
