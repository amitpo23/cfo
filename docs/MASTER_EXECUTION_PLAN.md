...
| org2/org3 | 🔴 **נשרו מהלולאה בשקט** — `integration_connections.status`: org2 `paused` מ-17/07 (467 שעות), org3 `inactive` מ-06/07 (762 שעות). לא "ממתינים לסקייל" — הופסקו בלי התרעה. ראה `reports/2026-08-05-cross-org-consistency-audit.md` |
...
|---|---|---|---|
| 0.1 | **deploy לפרוד** של ה־PR המאושר → זרימת הסכימה המאומתת ב־`GATE0_DEPLOYMENT_RUNBOOK.md` → smoke | בעלים + מפעיל מורשה בלבד | `remaining` ריק, revision=head, prod_smoke ירוק; מחזור בוקר רץ למחרת עד 06:45 על הנתונים הקיימים |
