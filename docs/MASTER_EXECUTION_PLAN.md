...
| org2/org3 | 🔴 **נשרו מהלולאה בשקט** — `integration_connections.status`: org2 `paused` מ-17/07 (467 שעות), org3 `inactive` מ-06/07 (762 שעות). לא "ממתינים לסקייל" — הופסקו בלי התרעה. הראיה: `reports/2026-08-05-cross-org-consistency-audit.md` — **מקומי בלבד, אינו ב-git** (מכיל שמות לקוחות ומזהי חברות). לשחזור: `scripts/roster_health.py` |
...
| שלמות נתונים (מדוד בפרוד 05/08) | 🔴 415 טיוטות בסכום 0 (org1 243, org2 16, org5 156) · 1,200 מתוך 1,491 הוצאות בלי ח.פ ספק · 8 חשבוניות org5 בלי איש קשר · `journal_entries`=0 **בכל הארגונים** |
