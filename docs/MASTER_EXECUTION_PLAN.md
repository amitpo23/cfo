...
|---|---|---|---|
| 0.1 | **deploy לפרוד** של ה־PR המאושר → זרימת הסכימה המאומתת ב־`GATE0_DEPLOYMENT_RUNBOOK.md` → smoke | בעלים + מפעיל מורשה בלבד | 🟡 **חלקי.** פרוד עלה ב־23/08 ל־**`27f0f87c152f`**. ה־head הנוכחי **`3f8c2a1d9e70`** (P0-C, ‏24/08) טרם נפרס; השער הבא הוא אישור בעלים לשדרוג `27f0f87c152f` → `3f8c2a1d9e70` לפי הרונבוק ואימות בריאות אחרי הפריסה. רק באישור נפרד יבוצע מחזור ידני חי שצורך מכסות SUMIT/OF; לחלון החדש נדרש גיבוי טרי. |
