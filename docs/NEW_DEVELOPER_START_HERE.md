...
- הריפו: `https://github.com/amitpo23/cfo`. ענף הפיתוח הנוכחי: `fix/rezef-stabilization-20260906`. אין לעבוד מ־`main` כאילו הוא כולל את הענף: ב־23.09 הענף טרם מוזג. עדכון `main` שעוצר את cron SUMIT שולב בענף לקראת PR; לפני תחילת עבודה יש לבדוק את מצב ה־PR וה־CI ב־GitHub.
...
- סנדבוקס ענן נפרד לפי הגדרת הבעלים: `https://cfo-dev-sandbox.vercel.app`. מזהה פרויקט Vercel שלו שונה מזה של `cfo-2` ואומת מקומית. קוד ההרשמה/גישה אישית נמסרים בערוץ פרטי על ידי הבעלים; אין לשמור אותם ב־Git. ב־23.09 Vercel דיווח `Ready`, בדיקת בריאות החזירה HTTP 200, `database=ok`, ו־Alembic `c81e6395fd71`; כניסה לחשבון הבדיקה הקיים החזירה 200. ניסיון לרשום את כל הארגונים עם אותו חשבון החזיר 403, לכן אין לו גישת `SUPER_ADMIN` לפלטפורמה. בידוד המסד עצמו, הרשמה ויצירת חשבון אישי טרם אומתו עצמאית.
- בבדיקת פריסת הסנדבוקס מ־22.09, Vercel עדיין הציג את `sync-sumit` כ־cron יומי, אף שהוא הוסר מ־`main` בהוראת בעלים. ברשימת **שמות** משתני הסביבה של פרויקט הסנדבוקס לא הופיע מפתח SUMIT (הערכים לא נקראו). לפני חיבור ספק, יש לבדוק את תצורת הפריסה האחרונה ולא להניח שהסנדבוקס משקף את הענף הנוכחי.
