...
**תאריך:** 24/08/2026. **מקור:** ייעוץ Fable, לפי הנחיית הבעלים. **סטטוס:** בביצוע — הבעלים אישר סדר: זהות מושקו + המדד קודם, WhatsApp אחרון.

## התקדמות

- ✅ **S9 (מדד המיקוד) — בוצע ונפרס (25/08/2026, commit `c96acae`).** `GET /api/admin/moshko/focus-metrics`. **בייסליין נמדד בפרוד לפני S5/S6:**
  - org2 (אליהב כהן): 8 תורי-assistant, 0% giveup, 1 gap פתוח (12.5/100 תורים), אין עדיין הרצת-רגרסיה.
  - כל הארגונים: 15 תורי-assistant, 0% giveup, 1 gap (6.67/100 תורים).
  - המספרים האלה הם קו-הבסיס להשוואה אחרי S5/S6.
