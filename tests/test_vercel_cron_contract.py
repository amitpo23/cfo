...
EXPECTED_DAILY_SCHEDULES = {
    ...
    #
    # /api/cron/sumit-quota-refresh — קוד קיים בענף הזה
    # (frozen/sumit-quota-refresh-pending-approval), **במכוון לא רשום כאן**.
    # זו אוטומציה שקוראת listquotas בפועל; נשארת קפואה עד שהבעלים יאשר
    # במפורש, אחרי שיתקבלו תשובות SUMIT (תקופת המכסה, מסלול בדיקות) ותוכנית
    # עבודה מסודרת. ר' docs/SUMIT_TESTING_DOCTRINE.md.
