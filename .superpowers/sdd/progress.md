...
# === 2026-07-12 (המשך) — מסמך-העל REZEF_OPERATING_MODEL ===
User reframed to first principles: Rezef = daily data brain over SUMIT+bank that replaces (a) the bookkeeper (capture/classify/reconcile/prepare filings) and (b) the CFO (P&L/cashflow/collections/insights), multi-client, Amit Porat first.
Wrote docs/REZEF_OPERATING_MODEL.md: essence (3 questions: completeness/truth/action), full mapping of both replaced roles to existing components, the DAILY RHYTHM (05:30 bank → hourly SUMIT → 06:00 completeness → 06:30 daily close → 07:00 collections → 08:00 morning brief; monthly: VAT package by 05, exec report 10th, filing check 15th), 5-metric per-client scorecard (capture≥95%, reconciliation≥90%, freshness≤24h, collections trend, tax-readiness≥5d) = definition of "full client", 3 rollout waves (A: Amit Porat T1-T7; B: journal batches M5 + closed missing-doc loop M4 + M7/M10; C: onboarding playbook + org5/org2/org3), honesty principle (reports state what they DON'T know), user blockers consolidated.

# === 2026-07-12 (המשך 2) — הרחבת תפקיד הנה"ח במסמך-העל ===
User: payment requests, document issuance, collections, VAT filings and TAX PAYMENTS are all part of the bookkeeper role Rezef replaces. Updated REZEF_OPERATING_MODEL.md: bookkeeper table expanded (document issuance via SUMIT documents API ✅wrapped; payment requests via sliqa redirect/OF payUrl/mandate ✅wrapped; collections; VAT filing; tax-payments calendar with approval-gated OF/Masav execution + bank-verified closure; supplier payments via Masav ✅exists). Monthly rhythm now includes the 15th "state-debts board" (file+pay VAT/advances/BL, owner approval, verify debit in bank). Wave B gained: tax&payments board (item 8) + proactive issuance for retainers via scheduleddocuments/recurring (item 9). Verified endpoints exist: /masav/{settings,preview,generate}, /payments/{charge,redirect/begin,recurring}, /accounting/documents{,/send,/pdf}, collections cases.

# === 2026-07-12 (ביצוע "צא לדרך") — דפדפן SUMIT books + כיול גבייה + בידוד OF ===
BROWSER (user's Chrome, logged-in SUMIT session):
- T2.1 FOUND: SUMIT books portal has a Tax Authority connection module (/accounting/shaamstatus, company 844329067): "חשבוניות ישראל" + "דיווחים מקוונים". Existing connection owner אליהב כהן (XXXXX5185), created by porat1921@gmail.com, **status לא פעיל — needs renewal (user-only action, tax-authority login)**.
- VAT screen works: /books/vat/?databaseid=2114194229. May-June 2026 report shows **מע"מ עסקאות 0, מע"מ תשומות ₪18,136 (החזר)** — BUT breakdown by ref-date: only ₪3,265 is May-June (May ₪2,279, June ₪986); ₪14,871 is Dec-2025→Apr-2026 backlog included because this is the file's FIRST report ("סימון תנועות ישנות" decision pending — CRITICAL: if those periods were already claimed by the previous accountant, including them = double claim).
- PUZZLE: books index shows עמית פורת next VAT period = **07-08/26** (not 05-06/26) — as if May-June is already handled/skipped. User must clarify with the office whether 05-06/26 was filed outside SUMIT.
- T1.1 HARVEST: iterated the books "תיוק קבצים" screen (SUMIT AI 2.0 pre-fills) via JS loop clicking דילוג: **200 pending files harvested** → scratchpad/org1_books_harvest.txt (tx id|ref|date|amount|vat|supplier+ח.פ). Breakdown: May-June 2026 = 64 files ₪38,836 (est. input VAT ~₪5,924); Dec-2025 cluster ₪177,571 (single ₪177,000 file tx 2114196868 ח.פ 005257334 — needs human eyes); 13 files with no amount (AI failed). NOT written into Rezef rows — row-level matching to the 82 empty business-side drafts is only possible by date (ambiguous), and creating new rows risks double-count when files get filed in SUMIT and sync back. DECISION: deliver as report; fill Rezef only after user picks the תיוק path (T1.3).
CODE (TDD, committed):
...
# === 2026-07-12 (המשך 4) — "ספקים חסרי חשבונית" LIVE ===
Sonnet subagent (TDD, 10 tests) + controller review: suppliers_missing_invoices aggregation (merchantName-first, normalized-description fallback, generic transfers → unidentified_transfers bucket), GET /daily-reports/suppliers-missing-invoices (default 90d), bot tool get_suppliers_missing_invoices, new Rezef screen 'ספקים חסרי חשבונית' (route /suppliers-missing-invoices + nav, FileX icon). Commit 96ae615, suite 904, tsc+build clean, deployed (cfo-2-of4rfiptr).
LIVE org1 May-June: 98 suppliers ₪37,747 undocumented (est. lost VAT ₪5,758) + 11 unidentified transfers ₪30,951. Top: מאיר מחסני קרמיקה ₪6,962 (invoice EXISTS in pending files), מופת ₪2,792, פז ₪2,002, שופרסל ₪1,968, מי רמת גן ₪1,590.
NOTE: card transactions ARE included (1,610 מסטרקארד rows in bank_transactions); card_settlement exclusion only removes the consolidated checking debit (double-count prevention).

# === 2026-07-12 (המשך 5) — "מטרת ההעברה" על העברות לא מזוהות ===
User asked "אתה לא רואה את הנמענים?" → dug raw_data: purposeDescription lives in description.additionalInfo (string-JSON). Added _transfer_purpose, purpose on unidentified_transfers rows + in alert titles + frontend column "מטרת ההעברה". 905 tests, deployed, live-verified: שכ"ד ₪7,000, סייעת לגיא ₪1,376+₪1,824 (פרטי), צהרון ₪600, מקדמה מעקה ₪2,200, הו"ק לחשבון 12-656-030900. ₪11,800 (27/05) likely the recurring lawyer invoice. Remaining truly unknown: ~₪5,600 (17/05 ₪1,441, 11/06 ₪1,200, 29/06 ₪1,740+₪1,200).

# === 2026-07-12 (סיום) — תוכנית שלמות הנתונים בוצעה ופרוסה ===
Two Sonnet agents (backend 954t, frontend tsc+build) + controller review + 3 follow-up fixes (numeric currency codes '1'/'2'→ILS/USD; nonzero-total month_has_books — zero-draft leftovers no longer fake a zeros-month; of_balance_freshness measures OUR sync recency not the bank's loan referenceDate). Final suite 956. Deploys 2df0f84+96e→cfo-2-1m2vrp9uy. Prod data fixes applied: fix_bills_sign_status (1,125 signs, 1,294 type-15→PAID), currency (17 ILS, 3 USD), migrations b1c2d3e4f5a6+c2d3e4f5a6b7 via /api/admin/db/migrate.
LIVE BEFORE→AFTER (org1 overview): cash 0→-14,466 (עו"ש במינוס, אמיתי, as-of 10/07) + savings 28,459 + loans 28,932 + card 18,324; AP -947,035→2,199; pnl_month now 2026-06 (last real month) not fake-zero July; last_sync populated; data_quality: ok/0 issues (was 2, both real catches, both resolved); daily-close cron live (06:30) — first run: 4 orgs, 4 snapshots, 0 errors. Smoke 16/16.
T5.1 daily-close DONE (was pending) — snapshots table feeding trends. All 9 session tasks closed except T1.1 (awaiting user's תיוק-path decision).

# === 2026-07-12 (המשך) — מיפוי כיסוי מלא SUMIT+OF + תיקון דליפה חוצת-לקוחות ===
User asked for a full capability mapping of SUMIT+OF to verify everything is implemented. 2 parallel Explore agents systematically checked all 84 SUMIT endpoints and all 83 OF methods against actual code usage.
SUMIT: 73/84 L3 (live), 4 L0 (unwrapped: createbatch, tokenizesingleuse, deals/*), 4 L0* (wired+always-raise by design: creditguy/billing/load, getentitieshtml, openupayterminal, recurring/charge), 3 L1. New finding: creditguy/billing/load fails silently every hourly SUMIT sync (dead weight since OF covers bank txns — cleanup candidate, not urgent).
OF: 70/83 L3 (route-exposed) but only ~9 "deep" (route+service+logic): list_accounts, list_transactions, list_connections, create_connection, get_connection, get_monthly_report, get_extended_securities, create_payment. 13 L1. category.main/sub NOW used (bank_expense_gap, built today); closingBooked used; cardDueDate NEVER used (real cashflow gap); mandates 0 business usage despite 4 wired methods; PATCH transactions never called (no write-back loop).
CRITICAL LIVE FINDING (caught during mapping, same bug class as today's connection_id fix): get_monthly_report/get_extended_securities have NO connectionId param in the provider's OpenAPI spec — server-side aggregated by userId only. org1+org2 (real live clients, connected today) share the SAME real Financy userId (amitporat1981@gmail.com) — calling either endpoint would return merged data across both clients' banks. FIXED: _has_shared_of_identity() in open_finance.py → 409 on direct routes, graceful skip in insights/generate enrichment. 5 tests, suite 961 passed. Commit aa5382d, deployed (cfo-2-gtikf8jzn), live-verified 409 on both org1 and org2, smoke 16/16.
Root-cause fix (separate Financy userId per client) is a product/onboarding decision, not code — ensure_of_identity already assigns rezef-org-<id> to NEW orgs, but doesn't retroactively re-scope a bank connection already created under a shared personal Financy login.
Doc: docs/REZEF_CAPABILITY_COVERAGE_2026-07-12.md (full tables) replaces part א' of REZEF_MASTER_ORCHESTRATION_PLAN.md (2026-07-10, stale).

# === 2026-07-12 (המשך) — משמעת עלות API: תיקון creditguy/billing/load ===
User directive: minimize API calls against SUMIT/Open Finance — costs money, call only what's necessary. Acted immediately on the concrete waste found during today's mapping: SumitConnector.fetch_bank_transactions called creditguy/billing/load (always-fails-by-design per its own docstring) on EVERY hourly /cron/sync-sumit run for EVERY connected org, forever, for zero benefit (bank data comes from Open Finance). Fixed to a local no-op (no network call), behaviorally identical. 1 new test (test_sumit_connector_no_wasted_calls.py), suite 962 passed. Commit 92dd522, deployed (cfo-2-qjbzr8pyy), smoke 16/16.
Saved durable feedback memory api-call-cost-discipline.md — applies to future manual verification too (prefer internal DB/route reads over live external syncs when verifying a fix, unless fresh external data is actually needed).

# === 2026-07-13 — דיווח מע"מ מרצף (PCN874+מבנה אחיד) + יצוא Excel/PDF בכל המסכים ===
Two Sonnet subagents + controller-built screen, commit 0dce100, deployed (cfo-2-p1ymj2ng5), 975 tests, tsc+build clean.
VAT FILING: select_vat_documents/period_bounds canonical selection (sales by doc date always; inputs by created_at when basis=captured); vat_report_period w/ monthly breakdown + document drill-down; /daily-reports/vat gains months(1|2)+basis; PCN874 rebuilt on same selection + /pcn874/file download; NEW openfrmt.py (מבנה אחיד INI+BKMVDATA, A100/C100/D110/D120/B100-real-only/Z900, ISO-8859-8 ZIP) via /daily-reports/openfrmt; new screen /vat-report (period+basis pickers, tiles, breakdown, documents, downloads). DRAFT disclaimers kept (validate once against ITA file checker).
...
# === 2026-07-13 (המשך) — מרכז ידע SUMIT מלא: 609 מאמרים ===
User directive: read ALL SUMIT help-center articles, become the top expert on usage+rules. 6 Sonnet agents read the entire help.sumit.co.il (609 articles; WAF 403 bursts bypassed via slow serial fetch / r.jina.ai / browser — zero SUMIT API calls). Output: docs/sumit_help_kb/01..06 (~4,300 lines, per-article digests) + docs/SUMIT_KNOWLEDGE_BASE.md (master: 34-rule rulebook + Rezef implications) + memory sumit-knowledge-base.
Key decision-changing rules: obligo=collection-debt mechanism ("פריקת חוב" fails→system blocked; release=pay/upgrade only); תיוק queue caps at 200 visible (Amit may have hidden drafts!); expense classified by UPLOAD date (validates captured-basis); final docs immutable (credit-only corrections); SHAAM connection expires every 3 months (add recurring reminder); card void=same-day+full-amount only; Masav silent-fail on bad bank details; spam law ₪226K (opt-in gate mandatory); blocked account risks data loss (Rezef's copy is the backup).

# === 2026-07-13 (המשך) — תיקוני אודיט אליהב + שער טריות מבוסס-checkpoints ===
Sonnet agent (TDD, f5737c9): zero-drafts filtered at select_vat_documents (all engines); sync-freshness gate in verification check 3; supplier tax-id on L rows (Contact.tax_id); basis_note honesty for captured basis. 1004 tests.
LIVE-CAUGHT gap right after deploy: run-based freshness gate missed org1's frozen pulls — hourly runs record COMPLETED even when circuit breaker skips all entities. Fixed (ca134b4): gate reads max(SyncCheckpoint.last_success_at) over sumit invoices/bills, surfaces open circuit window, SyncRun fallback only without checkpoints. 1006 tests, deployed.
VERIFIED against live prod data (local run over prod DB read-only — Vercel Security Checkpoint currently 403s curl on API paths; browsers unaffected): org1 = warn with "משיכת המסמכים מעולם לא הצליחה + הסנכרון מושהה עד 17:01 בגלל חסימת API — אין להגיש"; org2 = fresh (12:01 real success), warns only on 8 pending drafts. Checkpoint truth also revealed: obligo block is currently hitting org1 only; org2/org5 synced successfully at 12:01 — the block is per-usage/intermittent, not account-frozen-forever.
NOTE: Vercel bot-protection checkpoint on curl started after today's request volume; monitor — if it persists for real users, check Vercel Firewall/Attack Challenge settings.

# === 2026-07-13 (לילה) — השלמת P0+P1 מהתוכנית ===
FABLE browser audit: Amit's real filing queue = 262 files (200-cap bypassed via "כל הקבצים לתיוק" grid /f2114194212/v839662703/ with infinite scroll; 259 from the 05/07 bulk + 3 recent). Books VAT unchanged (18,136 incl backlog). docs/audits/2026-07-13-amit-filing-queue-depth.md. 62 files beyond our 200-file harvest remain unharvested.
...
# === 2026-07-17 — המשך: מנה 4 נותחה (ייחוס תוקן: org5!) + סטטוס ===
Session resumed. Suite 1,082 green. Committed leftovers (RSF-030 done mark, landing refresh).
BATCH-4 ANALYSIS (Fable+browser, read-only, docs/audits/2026-07-17-eliav-batch4-analysis.md): ATTRIBUTION FIX — batch 4 belongs to org5 עומר-ועודד (db 2114195556), not אליהב; the bookkeeper name on batches is "אליהב כהן" (source of the mislabel). Batch 4 = 69 rows ₪731,810 (not ₪5,983 — that's the dup amount). Findings: 14 dup pairs vs closed batch 2 = ₪5,948.44 to remove (VAT ₪907); electricity ₪28,209 income-receipt booked as EXPENSE (matches income invoice 20002 exactly — over-deduction exposure ≈₪5,210 VAT); Ituran ×3; 39/69 one-sided rows (balance -188,766) block closure; 4 income rows in expense batch (₪24,711); 2 rows from 2025 beyond 6-month window. SUMIT's built-in inter-batch dup checker returned ZERO vs 13 layer-1 pairs — proven unreliable again. 10 numbered owner decisions in the report; NO deletions/closure performed (owner judgment).
