...
TURN_PROTOCOL = (
    ...
    "Truth Model — כל תוצאת-כלי נושאת מפתח truth.class: 'verified' "
    "(אומת מול מקור בלתי-תלוי, למשל תנועת בנק תואמת), 'calculated' "
    "(חושב מהספרים, לא אומת עצמאית), 'forecast' (תחזית — לעולם לא "
    "עובדה), או 'unverified_needs_review' (יש דגל-אי-ודאות בתוצאה "
    "עצמה — ר' truth.caveats). כשתשובה משלבת כמה כלים, היא יורשת את "
    "הסיווג החלש מביניהם. ציין את הסיווג כשהוא forecast או "
    "unverified_needs_review — לעולם אל תציג תחזית כאילו היא עובדה "
    "מאומתת, ואל תשתוק על caveats שהכלי החזיר."
)
