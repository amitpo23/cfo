...
SCREENS: tuple[ScreenEntry, ...] = (
    ...
    ScreenEntry("/admin-moshko", "ניטור מושקו",
                "שיחות, קריאות כלים, טוקנים ועלות — למנהל־על בלבד.",
                "/api/admin/moshko/conversations"),
    ScreenEntry("/admin-moshko-knowledge", "הידע של מושקו",
                "ניהול הזיכרון הלומד וצפייה לקריאה בלבד בידע המקצועי.",
                "/api/admin/moshko/knowledge"),
...
def _reports_section() -> str:
    return (
        ...
        "burn-rate/נזילות) — **ספרים חיים** (`LiveCashFlowService`: "
        "BankTransaction בפועל + יתרת Account חיה; `/liquidity-ratios` גם "
        "היא Account.balance חי), לא טבלת התנועות הקפואה שהוחלפה ב-21/08/2026; "
        "honest-null כשאין תנועות בנק/יתרה חיה בטווח, לא ₪0 בביטחון-שווא.\n"
        ...
        "ולחלופה האמינה.\n"
        "- **תחזיות** (`/forecasting`) — **ספרים חיים** (`LiveForecastService`: "
        "חשבוניות/חשבונות-ספק פתוחים לפי due_date + ממוצע הוצאות חוזרות; "
        "אין ML/החלקה — סכימה ישירה בלבד), לא טבלת התנועות הקפואה.\n"
        "- **תקציב מול בפועל** (`/budget`) — עדיין חלקי (טבלת תנועות קפואה)."
    )
...
def _limitations_section() -> str:
    return (
        ...
        "- **`/budget`, ולשונית אנומליות ב-`/ai-analytics`** — עדיין מבוססים "
        "טבלת תנועות ישנה שהפסיקה לגדול מסנכרון חי; לא משקפים פעילות עדכנית. "
        "(`/cashflow-detail` ו-`/forecasting` הוחלפו ב-21/08/2026 לספרים "
        "חיים — `LiveCashFlowService`/`LiveForecastService` — ואינם עוד "
        "בהגבלה הזו.)\n"
