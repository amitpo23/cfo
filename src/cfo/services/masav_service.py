...
# קודי בנק/מוסד פעילים לפי רשימת חברי מס"ב (masav.co.il/participants-list),
# נשלפו ואומתו ב-2026-07-04 (43 קודים, ללא pagination/חיתוך - נספרו מול תגית
# "קוד בנק:" בעמוד המקור). קוד בן 2 ספרות, מרופד באפסים לבדיקה.
#
# הערת תחזוקה: בנק ישראל הכריז (13.1.26) על הרחבת קוד הזיהוי מ-2 ל-3 ספרות
# לצורך גישת גופים חוץ-בנקאיים למערכות התשלומים; לפי ההודעה הרשמית המעבר
# בפועל צפוי רק במהלך 2026-2027 (שימוש בפועל מ-~ינואר 2027). נכון להיום
# (2026-07-04) הקוד עדיין בן 2 ספרות - יש לעדכן רשימה זו כשהמעבר ייכנס לתוקף.
MASAV_PARTICIPANT_BANK_CODES = frozenset({
    "01", "03", "04", "06", "09",
...
def is_valid_bank_code(bank_code: str) -> bool:
    """בדיקת קוד בנק/מוסד מול רשימת חברי מס"ב הפעילה (ר' MASAV_PARTICIPANT_BANK_CODES)."""
    text = str(bank_code).strip()
    if not text.isdigit():
        return False
